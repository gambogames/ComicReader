...
- (IBAction)closeModalSettings:(id)sender {
    
    [_menuControllerIPhone dismissViewControllerAnimated:YES completion:nil];
}
...
- (void) _toggleMenu:(id)sender {
    ...
    if ( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad )
    {
        ...
        [self presentViewController:_menuControllerIPhone animated:YES completion:nil];
...
- (void) viewDidLoad {
  ...
  viewController.view = _menuView;
    ...
    if ( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad )
    {
        ...
        UIBarButtonItem* rightButtonDropBox = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"DROPBOX_BUTTON", nil)
                                                                        style:UIBarButtonItemStyleBordered
                                                                       target:self
                                                                       action:@selector(syncDropbox)];
        item.rightBarButtonItems = @[rightButton,rightButtonDropBox];
        [rightButton release];

    } else {
        
        _menuControllerIPhone = [[UIViewController alloc] init];
        _menuView.frame = [UIScreen mainScreen].bounds;
        NSLog(@"%@", NSStringFromCGRect(_menuView.frame));
        _menuView.backgroundColor = [UIColor whiteColor];
        [_menuControllerIPhone.view addSubview: _menuView];
        
        [self.btnSettings addTarget:self action:@selector(_toggleMenu:) forControlEvents:UIControlEventTouchUpInside];
        [self.btnSettings setTitle:NSLocalizedString(@"SETTINGS_BUTTON", nil) forState:UIControlStateNormal];
        
        [self.btnSyncDropbox addTarget:self action:@selector(syncDropbox) forControlEvents:UIControlEventTouchUpInside];
        [self.btnSyncDropbox setTitle:NSLocalizedString(@"DROPBOX_BUTTON", nil) forState:UIControlStateNormal];
    }
...
- (void)syncDropbox {
    ...
    dropboxBrowser.allowedFileTypes = @[ @"pdf", @"zip", @"cbz", @"rar", @"cbr"];
  ...
  if (!_gridView.empty) {
    ...
    if (_currentCollection) {
      if (scrolling != _currentCollection.scrolling) {
        _currentCollection.scrolling = (int)scrolling;
