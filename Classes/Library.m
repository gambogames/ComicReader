...
- (void) _updateComicForPath:(NSString*)path
  ...
  if (force == NO) {
    ...
    if (data.length == sizeof(DatabaseSQLRowID)) {
      ...
      if (rowID == kFakeRowID) {
        ...
        comic = (Comic*)CFDictionaryGetValue(zombieComics, rowID);
      }
    }
  }
  ...
  // If yes, update comic
  if (comic) {
    CFDictionaryRemoveValue(zombieComics, comic.sqlRowID);
...
- (id) _updateLibrary:(BOOL)force {
  ...
  for (Collection* collection in [connection fetchAllObjectsOfClass:[Collection class]]) {
    CFDictionarySetValue(zombieCollections, collection.sqlRowID, collection);
  }
  ...
  for (Comic* comic in [connection fetchAllObjectsOfClass:[Comic class]]) {
    CFDictionarySetValue(zombieComics, comic.sqlRowID, comic);
  }
  ...
  // Process directories
  {
    ...
    for (NSString* directory in directories) {
      ...
      if (directory.length && !force) {
        ...
        if (data.length == sizeof(DatabaseSQLRowID)) {
          DatabaseSQLRowID rowID = *((DatabaseSQLRowID*)data.bytes);
          collection = (Collection*)CFDictionaryGetValue(zombieCollections, rowID);
        }
      }
      
      // If yes, update collection
      if (collection) {
        CFDictionaryRemoveValue(zombieCollections, collection.sqlRowID);
        if (time != collection.time) {
          needsUpdate = YES;
        } else {
          CFDictionaryApplyFunction(zombieComics, _ZombieComicsMarkFunction, collection.sqlRowID);
